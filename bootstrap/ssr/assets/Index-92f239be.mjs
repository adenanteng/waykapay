import { ref, watch, mergeProps, withCtx, unref, isRef, createTextVNode, createVNode, toDisplayString, openBlock, createBlock, Fragment, renderList, useSSRContext } from "vue";
          ...
          if (_push2) {
            ...
            ssrRenderList(props.users.data, (item) => {
              ...
              _push2(ssrRenderComponent(unref(Link), {
                ...
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`<div class="px-4 py-4 sm:px-6"${_scopeId2}><div class="flex items-center justify-between"${_scopeId2}><p class="text-sm font-medium truncate capitalize text-gray-900"${_scopeId2}>${ssrInterpolate(item.name)} </p><div class="ml-2 flex-shrink-0 flex"${_scopeId2}><p class="${ssrRenderClass([item.color, "px-2 inline-flex text-xs leading-5 font-semibold rounded-full"])}"${_scopeId2}>${ssrInterpolate(item.status)}</p></div></div><div class="flex justify-between"${_scopeId2}><div class="sm:flex gap-3"${_scopeId2}><p class="flex items-center text-xs text-gray-600"${_scopeId2}>${ssrInterpolate(item.phone)}</p><p class="flex items-center text-xs text-gray-600"${_scopeId2}>${ssrInterpolate(item.email)}</p></div><div class="flex items-end text-xs text-gray-600"${_scopeId2}>${ssrInterpolate(formattedDate(item.updated_at))}</div></div></div>`);
                  } else {
                    return [
                      createVNode("div", { class: "px-4 py-4 sm:px-6" }, [
                        createVNode("div", { class: "flex items-center justify-between" }, [
                          createVNode("p", { class: "text-sm font-medium truncate capitalize text-gray-900" }, toDisplayString(item.name) + " ", 1),
                        ...
                        createVNode("div", { class: "flex justify-between" }, [
                          createVNode("div", { class: "sm:flex gap-3" }, [
                            createVNode("p", { class: "flex items-center text-xs text-gray-600" }, toDisplayString(item.phone), 1),
                            createVNode("p", { class: "flex items-center text-xs text-gray-600" }, toDisplayString(item.email), 1)
                          ]),
                          createVNode("div", { class: "flex items-end text-xs text-gray-600" }, toDisplayString(formattedDate(item.updated_at)), 1)
                        ])
                      ])
                    ];
                  }
